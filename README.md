...
| `colorScale` | `"absolute"\|"share"` | `"absolute"` | 表示と色の基準（下記） |
...
| 値 | セルの表示 | 色の基準 |
|---|---|---|
| `absolute`（実数） | 値そのもの（`format` 適用） | 全体の最大値 |
| `share`（比率） | **構成比(%)** | そのブロック内の最大値 |

売上のようなロングテールのデータは、全体の最大値で正規化すると
**上位1件だけが濃くて残りが全部ほぼ白**になり、構造が読めなくなる。
`share` にすると各ブロックが自分の中の最大値で正規化されるので、
どのブロックでも「その中で何が大きいか」が色で読める。

構成比の分母は階層によって変わる。

- **アイテム**（ブロック内の8マス）… そのブロックの合計。8マスの合計が100%になる
- **グループ**（ブロック中央・中心ブロック）… 全体の総計
- **総計**（中心のマス）… 常に `100%`

実数モードでもツールチップには構成比が出るので、
「色は比率で見つつ、実額はホバーで確認する」という読み方ができる。
